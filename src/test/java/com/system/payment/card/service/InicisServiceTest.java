...
//
//    @InjectMocks InicisService inicisService;
//
//    @Test
//    @DisplayName("PG 팝업 파라미터 생성 - 해시/필드 값 모두 검증")
//    void createBillingAuthParams_buildsParamsWithCorrectHashesAndFields() {
//        // given
//        final String oid = IdGeneratorUtil.oidGenerate();
//        final String ts = IdGeneratorUtil.timestampGenerate();
////        final String ts = "1712345678000";
////        when(idGeneratorUtil.timestampGenerate()).thenReturn(ts);
//
...
//
//        // when
//        InicisBillingAuthResponse res = inicisService.createBillingAuthParams(oid, req);
//
//        // then: 고정 필드
...
//        assertThat(res.getOid()).isEqualTo(oid);
...
//        String sigText = String.format("oid=%s&price=%s&timestamp=%s", oid, PRICE_ZERO, ts);
//        String varText = String.format("oid=%s&price=%s&signKey=%s&timestamp=%s", oid, PRICE_ZERO, SIGN_KEY, ts);
